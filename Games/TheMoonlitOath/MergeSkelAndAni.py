...
TARGET_DIR = r"D:\Tools\UsefulTools\MuMu\Shared\Download\花亦山\output\TMP"
...
def load_spineani_animations(sk, spineani_files):
    """
    接收已经匹配到的 spineani 文件路径列表并进行解析
    """
    loaded_files = []

    for fp in spineani_files:
        try:
...
def process_directory(target_dir):
    skel_files = glob.glob(os.path.join(target_dir, "*.skel"))

    if not skel_files:
        print(f"[INFO] 在目录 {target_dir} 中未找到任何 .skel 文件。")
        return

    print(f"[INFO] 找到 {len(skel_files)} 个 .skel 文件，开始筛选并处理...")

    for skel_path in skel_files:
        base_name = os.path.splitext(os.path.basename(skel_path))[0]

        # 1. 提前探测：查找是否有对应的 spineani 文件
        pattern = os.path.join(target_dir, f"{base_name.lower()}__*.spineani")
        spineani_files = glob.glob(pattern)

        # 如果没有找到对应的 spineani 文件，则跳过不处理
        if not spineani_files:
            continue

        out_json_path = os.path.join(target_dir, f"{base_name}.json")
        print(f"\n--- 正在处理: {base_name} ---")
        ...
        try:
            ...
            # 3. 加载刚才探测到的 spineani 动画文件
            loaded_files = []
            if not sk.animations:
                loaded_files = load_spineani_animations(sk, spineani_files)
            ...
            # 5. 清理阶段：确认 JSON 写入成功后，删除源文件
            os.remove(skel_path)

            for sp_file in loaded_files:
                os.remove(sp_file)

        except Exception as e:
            print(f"  [ERROR] 处理 {base_name} 失败: {e}")
