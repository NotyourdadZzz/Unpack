...
1. `DecryptXOR.py`/`DecryptXORTest.py`： 功能一致，Test是前者的完全版，检测Bundle文件的异或加密种类，可以尝试更多种类的XOR加密，用于处理unity资源的XOR加密。
2. `Decrypt64B-XOR(GirlsLoveDance).py`：处理 千娇百媚 的文件前64个字节XOR加密
3. `WPKUnpacker.py`：解密WPK文件，通常会得到LPK、config、png；调用了本地bandizip进行解压，使用了bandizip6.29的bc.exe命令行工具，高级版本为bz.exe，需要把bc添加到环境变量
4. `LPKUnpacker.py`：解密LPK文件，需要配合config.json文件一起使用；支持Live2D和Spine 
5. `YooAssetUnpacker.py`：用于处理Unity的YooAsset资产框架解密。经典目录结构就是Package目录下有ManifestFiles和CacheBundleFiles，处理完之后就是正常的unity文件
6. `DecryptOrisries.py`：用于处理万源圣魔录得资源加密，处理后输出到decrypted目录下。
7. `LZ4Dec.py`：用于LZ4解压，需要参数指定文件，解压后输出txt
...
1. `MergeSkel(XiYou).py`：内部设置_aniinfo文件路径，查找bundles路径，输出路径

   通常把_aniinfo文件全部按路径导出后就会在对应spine模型目录下，bundles路径就是所有u2d文件的路径，输出路径通常和\_aniinfo文件路径一致，成功合成的skel文件会输出到对应spine目录下面。
